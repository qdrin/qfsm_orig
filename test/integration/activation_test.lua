...
g.test_trial_price_past_event_date = function()
  local mstate = helper.build_machine_state("pendingActivate")
  local price = helper.prices.offer1.offer1PriceTrial
  ...
  local expected = {
    ...
    product = {
      status = "ACTIVE_TRIAL",
      activeEndDate = function(real)
        local dt = real - helper.end_of_price(price, t0)
        return dt >= -10 and dt < 10
      end,
      trialEndDate = function(real)
        local dt = real - helper.end_of_price(price, t0)
    ...
    tasks = {
      {
        ...
        extraParams = {eventType = "trial_ended"},
        wakeAt = function(real)
          local dt = real - (helper.end_of_price(price, t0) - helper.config.price_ended_before_time)
          return dt >= -10 and dt < 10
        end,
      }
    },
...
g.test_commercial_price_past_event_date = function()
  local mstate = helper.build_machine_state("pendingActivate")
  local price = helper.prices.offer1.offer1PriceActive
  local t0 = os.time() - 86400*10
...
g.test_commercial_price_with_promo = function()
  local mstate = helper.build_machine_state("pendingActivate")
  local price = helper.prices.offer1.offer1PriceActive
  local t0 = os.time()
  local t1 = helper.end_of_price(price)
