...
g.test_normal = function()
  ...
  local expected = {
    ...
    delete_tasks = {
      {wakeAtFunction = "send_event", extraParams = {eventType = "trial_ended", price = nil}, wakeAt = nil},
  ...
  local res, err = helper.send_event("payment_failed", data, "p1")
  print("result: ", json.encode(res), "err", err)
  t.assert(res, err)
  helper.assert_result(res, expected)
end

g.test_first_tarification = function()
  local mstate = helper.build_machine_state("active", "waitingPayment", "commercial")
  local price = helper.prices.offer1.offer1PriceActive
  -- local new_price = table.deepcopy(price)
  price.period = 1
